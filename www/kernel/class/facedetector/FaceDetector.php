...
class FaceDetector
{
    ...
    {
		$detection_data = 'FaceDetector.dat';
    ...
    public function picHasFace($resource)
    {
        $this->faceDetect($resource);
        if (empty($this->face) || $this->face == null) {
            return false;
        }
        else
        {
    ...
    public function cropFaceToJpegResizer($outFileName = null, $size = null)
    {
        if($size === null)
        {
                $size = $this->face['w'];
        }
        $sizeOfX = $this->face['x']+($this->face['x']-$size);
        $sizeOfY = $this->face['y']+($this->face['y']-$size);
        $finalImage = $this->canvas;
        $maxX = imagesx($finalImage);
        $maxY = imagesy($finalImage);
        if($sizeOfX < 0)
        {
            $sizeOfX = 0;
        }
        ...
        {
            $finalImage = imagescale($finalImage,$size,$size);
        }
        if (empty($this->face)) {
            throw new \Exception('No face detected');
        }
        ...
        imagecopy($canvas, $finalImage, 0, 0, $sizeOfX, $sizeOfY, $size, $size);

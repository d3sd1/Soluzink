...
<html lang="<?php echo $alang['meta.lang'] ?>">
    ...
    <head>
        ...
        <title><?php echo $alang['title.users'] ?></title>
    ...
    <body class="" data-toggle="context" data-target="#context-menu">
        ...
        <div class="container-fluid">
			...
			<li><?php echo $alang['path.users'] ?></li>
                    </ul>
                    <!-- END BREADCRUMBS -->
                ...
                <div class="page-fixed-main-content">
                    ...
                    <?php
                    if(isset($_GET['action']) && $_GET['action'] != null && $_GET['action'] != '' && isset($_GET['id']) && $_GET['id'] != null && $_GET['id'] != '')
                    {
                        $decryptId = $core->decrypt($_GET['id']);
                        if($decryptId != '' && $decryptId != null)
                        {
                            if($db->query('SELECT id FROM users WHERE id='.$decryptId)->num_rows > 0)
                            {
                                if($_GET['action'] == 'ban')
                                {
                                    $db->query('INSERT INTO admin_logs (type,ip,userId,timestamp) VALUES ("BANNED_USER",INET_ATON(\''.USER_IP.'\'),(SELECT id FROM admin_users WHERE email="'.ADMIN_EMAIL.'"),"'.time().'")');   
                                    $db->query('UPDATE users SET banned=1 WHERE id='.$decryptId);
                                    echo '<div class="alert alert-success fade in alert-dismissable" style="margin-top:18px;">
                                            <a href="#" class="close" data-dismiss="alert" aria-label="close" title="close">×</a>
                                            <strong>¡Éxito!</strong> Has expulsado al usuario correctamente.
                                    </div>';
                                }
                                elseif($_GET['action'] == 'unban')
                                {
                                    $db->query('INSERT INTO admin_logs (type,ip,userId,timestamp) VALUES ("UNBANNED_USER",INET_ATON(\''.USER_IP.'\'),(SELECT id FROM admin_users WHERE email="'.ADMIN_EMAIL.'"),"'.time().'")');   
                                    $db->query('UPDATE users SET banned=0 WHERE id='.$decryptId);
                                    echo '<div class="alert alert-success fade in alert-dismissable" style="margin-top:18px;">
                                            <a href="#" class="close" data-dismiss="alert" aria-label="close" title="close">×</a>
                                            <strong>¡Éxito!</strong> Has admitido al usuario correctamente.
                                    </div>';
                                }
                            }
                            else
                            {
                                echo '<div class="alert alert-danger fade in alert-dismissable" style="margin-top:18px;">
                                        <a href="#" class="close" data-dismiss="alert" aria-label="close" title="close">×</a>
                                        <strong>¡Error!</strong> El usuario introducido no existe.
                                </div>';
                            }
                        }
                        else
                        {
                            echo '<div class="alert alert-danger fade in alert-dismissable" style="margin-top:18px;">
                                        <a href="#" class="close" data-dismiss="alert" aria-label="close" title="close">×</a>
                                        <strong>¡Error!</strong> Ocurrió un error al expulsar al usuario.
                                </div>';
                        }
                    }
                    ?>
                        ...
                        <tbody>
                            <?php
                            $usersMgr = $db->query('SELECT id,email,name,surnames,lang,geoCity,geoTimeZone,lastIp,dateLastLogin,type,banned FROM users');
                            while($row = $usersMgr->fetch_row())
                            {
                                switch($row[9])
                                {
                                    case 'client':
                                    ...
                                    break;
                                    case 'psico':
                                        $clientType = 'Psicólogos';
                                    break;
                                    case 'coach':
                                        $clientType = 'Motivador';
                                    break;
                                }
                                echo '<tr>
                                        ...
                                        <td class="text-center">'.$core->timeElapsed($row[8]).'</td>
                                        <td class="text-center">'.$clientType.'</td>
                                        '.($row[10] ? '<td class="text-center"><a href="'.URL.'/users?action=unban&id='.$core->crypt($row[0]).'" class="btn btn-info">Admitir usuario</a></td>':'<td class="text-center"><a href="'.URL.'/users?action=ban&id='.$core->crypt($row[0]).'" class="btn btn-danger">Expulsar usuario</a></td>').'
                                      </tr>';
                            }
                            ?>
                        </tbody>

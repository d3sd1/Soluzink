            ...
            <div class="page-content page-content-popup">
                ...
                <div class="page-sidebar-wrapper">
                    ...
                    <div class="page-sidebar navbar-collapse collapse">
                        ...
                        <?php echo $admin->menu('patologies'); ?>
                        <!-- END SIDEBAR MENU -->
                    </div>
                    <!-- END SIDEBAR -->
                </div>
                <div class="page-fixed-main-content">
                    ...
                    <h2><a href="<?php echo URL ?>/addPatology" class="btn btn-primary">Agregar patología</a></h2>
                    <?php
                    if(isset($_GET['des']))
                    {
                        $patToDel = $db->query('SELECT id FROM patologies WHERE id='.$_GET['des']);
                        if($patToDel->num_rows > 0)
                        {
                            $db->query('UPDATE patologies SET active=0 WHERE id='.$_GET['des']);
                            $db->query('INSERT INTO admin_logs (type,ip,userId,timestamp) VALUES ("PATOLOGY_DISABLED",INET_ATON(\''.USER_IP.'\'),(SELECT id FROM admin_users WHERE email="'.ADMIN_EMAIL.'"),"'.time().'")');   
                            echo '<div class="alert alert-success fade in alert-dismissable" style="margin-top:18px;">
                                    <a href="#" class="close" data-dismiss="alert" aria-label="close" title="close">×</a>
                                    <strong>¡Éxito!</strong> Has desactivado la patología correctamente.
                            </div>';
                        }
                        else
                        {
                            $db->query('INSERT INTO admin_logs (type,ip,userId,timestamp) VALUES ("PATOLOGY_UPDATE_ERROR",INET_ATON(\''.USER_IP.'\'),(SELECT id FROM admin_users WHERE email="'.ADMIN_EMAIL.'"),"'.time().'")');   
                                echo '<div class="alert alert-danger fade in alert-dismissable" style="margin-top:18px;">
                                        <a href="#" class="close" data-dismiss="alert" aria-label="close" title="close">×</a>
                                        <strong>¡Error!</strong> La patología introducida no se pudo desactivar.
                                </div>';
                        }
                    }
                    if(isset($_GET['act']))
                    {
                        $patToDel = $db->query('SELECT id FROM patologies WHERE id='.$_GET['act']);
                        if($patToDel->num_rows > 0)
                        {
                            $db->query('UPDATE patologies SET active=1 WHERE id='.$_GET['act']);
                            $db->query('INSERT INTO admin_logs (type,ip,userId,timestamp) VALUES ("PATOLOGY_ENABLED",INET_ATON(\''.USER_IP.'\'),(SELECT id FROM admin_users WHERE email="'.ADMIN_EMAIL.'"),"'.time().'")');   
                            
                            echo '<div class="alert alert-success fade in alert-dismissable" style="margin-top:18px;">
                                    <a href="#" class="close" data-dismiss="alert" aria-label="close" title="close">×</a>
                                    <strong>¡Éxito!</strong> Has activado la patología correctamente.
                            </div>';
                        }
                        else
                        {
                            $db->query('INSERT INTO admin_logs (type,ip,userId,timestamp) VALUES ("PATOLOGY_UPDATE_ERROR",INET_ATON(\''.USER_IP.'\'),(SELECT id FROM admin_users WHERE email="'.ADMIN_EMAIL.'"),"'.time().'")');   
                                ...
                                echo '<div class="alert alert-danger fade in alert-dismissable" style="margin-top:18px;">
                                        <a href="#" class="close" data-dismiss="alert" aria-label="close" title="close">×</a>
                                        <strong>¡Error!</strong> La patología introducida no se pudo activar.
                                </div>';
                        }
                    }
                    ?>
                    <table class="table table-bordered">
                        <thead>
                          <tr>
                            ...
                            <th class="text-center">Clave de idioma</th>
                            <th class="text-center">Nombre de la patología</th>
                        ...
                        <tbody>
                            <?php
                            $adminsMgr = $db->query('SELECT id,langkey,active,type FROM patologies');
                            while($row = $adminsMgr->fetch_row())
                            {
                                echo '<tr>
                                        <td class="text-center">'.$row[0].'</td>
                                        <td class="text-center">'.$row[1].'</td>
                                        <td class="text-center">'.$lang['DB']['treatments'][$row[1]].'</td>
                                        <td class="text-center">'.($row[2] ? 'Activada':'Desactivada').'</td>
                                        <td class="text-center">'.$row[3].'</td>
                                        '.($row[2] ? '<td class="text-center"><a href="'.URL.'/adminsMgr?des='.$row[1].'" class="btn btn-danger">Desactivar patología</a></td>':'<td class="text-center"><a href="'.URL.'/adminsMgr?act='.$row[1].'" class="btn btn-danger">Activar patología</a></td>').'
                                      </tr>';
                            }
                            ?>
                        </tbody>

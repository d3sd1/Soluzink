            ...
            <div class="page-content page-content-popup">
                ...
                <div class="page-sidebar-wrapper">
                    ...
                    <div class="page-sidebar navbar-collapse collapse">
                        ...
                        <?php echo $admin->menu('sessionsdone'); ?>
                        <!-- END SIDEBAR MENU -->
                    </div>
                    <!-- END SIDEBAR -->
                </div>
                <div class="page-fixed-main-content">
                    <h1>Sesiones realizadas</h1>
                    <?php
                    if(isset($_GET['error']) && $_GET['error'] == 'chatnotfound')
                    {
                    ...
                    <table class="table table-bordered">
                        <thead>
                          <tr>
                            ...
                            <th class="text-center">Duración de la sesión</th>
                            <th></th>
                          </tr>
                        </thead>
                        <tbody>
                            <?php
                            $adminsMgr = $db->query('SELECT u.name,u.surnames,p.name,p.surnames,ups.timestamp,pt.langkey,ups.sessionId,ups.sessionlengthmins FROM users_psicos_sessions ups JOIN users u ON u.id=ups.user_id JOIN users p ON p.id=ups.psico_id JOIN patologies pt ON pt.id=ups.patid ORDER BY ups.timestamp DESC');
                            while($row = $adminsMgr->fetch_row())
                            {
                                echo '<tr>
                                        <td class="text-center">'.$row[0].' '.$row[1].'</td>
                                        <td class="text-center">'.$row[2].' '.$row[3].'</td>
                                        <td class="text-center">'.date('d/m/Y H:m:i',$row[4]).'</td>
                                        <td class="text-center">'.$lang['DB']['treatments'][$row[5]].'</td>
                                        <td class="text-center">'.$row[7].'</td>
                                        <td class="text-center"><a href="'.URL.'/sessionchats?key='.$core->crypt($row[6]).'" class="btn btn-primary">Ver logs del chat</a></td>
                                      </tr>';
                            }
                            ?>
                        </tbody>

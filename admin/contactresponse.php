...
{
    $contactCode = $core->decrypt($_GET['id']);
    if($contactCode != '' && $contactCode != null)
    {
        $contactInfo = $db->query('SELECT datatype,dataId,timestamp,solved,message,lang FROM contacts WHERE id='.$contactCode);
        if($contactInfo->num_rows == 0)
        {
            header('Location: '.URL.'/contacts?error=contactnotfound');
            die();
        }
        else
        {
            $contactDataInfo = $contactInfo->fetch_array();
            if($contactDataInfo[0] == 'contactId')
            {
                $contactUserData = $db->query('SELECT name,email,phone,INET_NTOA(ip) FROM contactdata WHERE id='.$contactDataInfo['dataId']) or die(header('Location: '.URL.'/contacts?error=contactnotfound'));
            }
            else
            {
                $contactUserData = $db->query('SELECT name,email,phone,INET_NTOA(lastIp) FROM users WHERE id='.$contactDataInfo['dataId']) or die(header('Location: '.URL.'/contacts?error=contactnotfound'));
            }
            $contactUserData = $contactUserData->fetch_row();
        }
    }
    else 
    {
...
{
    $db->query('INSERT INTO admin_logs (type,ip,userId,timestamp) VALUES ("CONTACT_ANSWERED",INET_ATON(\''.USER_IP.'\'),(SELECT id FROM admin_users WHERE email="'.ADMIN_EMAIL.'"),"'.time().'")');   
    $email->adminResponseContact($contactUserData[1],$contactUserData[0],$_POST['contactdata']);
    $db->query('UPDATE contacts SET solved=1 WHERE id='.$contactCode);
    header('Location: '.URL.'/contacts?success=true');
    die();
}
?>
<!DOCTYPE html>
            ...
            <div class="page-content page-content-popup">
                ...
                <div class="page-sidebar-wrapper">
                    ...
                    <div class="page-sidebar navbar-collapse collapse">
                        ...
                        <?php echo $admin->menu('contacts'); ?>
                        <!-- END SIDEBAR MENU -->
                    </div>
                    <!-- END SIDEBAR -->
                </div>
                <div class="page-fixed-main-content">
                    <h1>Responder solicitud de contacto</h1>
                    <h2><a href="<?php echo URL ?>/contacts" class="btn btn-primary">Volver a solicitudes</a></h2>
                    <table class="table table-bordered col-md-12" style="table-layout:fixed;">
                        <thead>
                          <tr>
                            ...
                            <th class="text-center col-md-3">Teléfono</th>
                            <th class="text-center col-md-3">IP</th>
                          </tr>
                        </thead>
                        <tbody>
                            <tr>
                                <td class="text-center col-md-3"><?php echo $contactUserData[0] ?></td>
                                <td class="text-center col-md-3"><?php echo $contactUserData[1] ?></td>
                                <td class="text-center col-md-3"><?php echo $contactUserData[2] ?></td>
                                <td class="text-center col-md-3"><?php echo $contactUserData[3] ?></td>
                            </tr>
                        </tbody>
                      </table>
                    <table class="table table-bordered col-md-12" style="table-layout:fixed;">
                        <thead>
                          <tr>
                            <th class="text-center col-md-4">Fecha</th>
                            <th class="text-center col-md-4">Mensaje</th>
                            <th class="text-center col-md-4">Idioma</th>
                          </tr>
                        </thead>
                        <tbody>
                            <tr>
                                <td class="text-center col-md-2"><?php echo date('d/m/Y H:m:i',$contactDataInfo['timestamp']) ?></td>
                                <td class="text-center col-md-9" style="white-space: nowrap;overflow: hidden;text-overflow: ellipsis"><?php echo $contactDataInfo['message'] ?></td>
                                <td class="text-center col-md-1"><?php echo $lang['TRANSCRIPTION'][$contactDataInfo['lang']] ?></td>
                            </tr>
                        </tbody>
                      </table>
                    ...
                    <a id="sendResponse" class="btn btn-success" style="margin-top: 18px">Responder solicitud</a>
                </div>
                <!-- BEGIN FOOTER -->
...
<![endif]-->
        <!-- BEGIN CORE PLUGINS -->
	<script src="<?php echo URL ?>/assets/apps/ckeditor/ckeditor.js"></script>
	<script src="<?php echo URL ?>/assets/apps/ckeditor/main.js"></script>
		...
		<script type="text/javascript">
                ...
                {
                    if(typeof CKEDITOR.instances.contactResponser.getData() != 'undefined' && CKEDITOR.instances.contactResponser.getData() != '' && CKEDITOR.instances.contactResponser.getData() != null)
                    {
                        $('input[name=\'contactdata\']').val(CKEDITOR.instances.contactResponser.getData());
                        $('#contactForm').submit();
                    }
                });
		(function (win)

        ...
        <div class="container-fluid">
            <div class="page-content page-content-popup">
                ...
                <div class="page-fixed-main-content">
                    <h2><a href="<?php echo URL ?>/patologies" class="btn btn-primary">Ver patologías</a></h2>
                    <?php
                    if(isset($_POST) && count($_POST) > 0)
                    {
                        if(isset($_POST['langkey']) && $_POST['langkey'] != null && $_POST['langkey'] != '' && isset($_POST['translate']) && $_POST['translate'] != null && $_POST['translate'] != '' && isset($_POST['status']) && isset($_POST['docenttype']))
                        {
                            if($db->query('SELECT id FROM patologies WHERE langkey="'.$_POST['langkey'].'"')->num_rows == 0)
                            {
                                $db->query('INSERT INTO admin_logs (type,ip,userId,timestamp) VALUES ("ADDED_PATOLOGY",INET_ATON(\''.USER_IP.'\'),(SELECT id FROM admin_users WHERE email="'.ADMIN_EMAIL.'"),"'.time().'")');
                                $db->query('INSERT INTO patologies (langkey,active,type) VALUES ("'.strtoupper($_POST['langkey']).'",'.$_POST['status'].',"'.$_POST['docenttype'].'")');
                                file_put_contents(subfolder.'/kernel/langs/es.php',file_get_contents(subfolder.'/kernel/langs/es.php').PHP_EOL.'$lang[\'DB\'][\'treatments\'][\''.strtoupper($_POST['langkey']).'\'] = \''.$_POST['translate'].'\';');
                            ...
                            {
                                $db->query('INSERT INTO admin_logs (type,ip,userId,timestamp) VALUES ("ADDED_PATOLOGY_ERROR",INET_ATON(\''.USER_IP.'\'),(SELECT id FROM admin_users WHERE email="'.ADMIN_EMAIL.'"),"'.time().'")');
                                echo '<div class="alert alert-danger fade in alert-dismissable">
                                        <a href="#" class="close" data-dismiss="alert" aria-label="close" title="close">×</a>
                                        <strong>¡Error!</strong> La clave de idioma introducida ya está en uso.
                                </div>';
                            }
                        }
                        else
                    ...
                    ?>
                    <form method="post" action="<?php echo URL ?>/addPatology">
                        <div class="form-group">
                          <label for="langkey">Clave de idioma:</label>
                          <input type="text" name="langkey" class="form-control">
                        </div>
                        <div class="form-group">
                          <label for="translate">Nombre de la patología (Español):</label>
                        ...
                        <div class="form-group">
                          <label for="docenttype">Docente</label>
                          <select class="form-control" name="docenttype">

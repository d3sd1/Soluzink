        ...
        <div class="container-fluid">
            <div class="page-content page-content-popup">
                ...
                <div class="page-fixed-main-content">
                    <h1>Solicitudes de contacto (Sin resolver)</h1>
                    <h2><a href="<?php echo URL ?>/closedcontacts" class="btn btn-primary">Solicitudes resueltas</a></h2>
                    <?php
                    if(isset($_GET['error']) && $_GET['error'] == 'contactnotfound')
                    {
                        echo '<div class="alert alert-danger fade in alert-dismissable" style="margin-top:18px;">
                                        <a href="#" class="close" data-dismiss="alert" aria-label="close" title="close">×</a>
                                        <strong>¡Error!</strong> No se pudo actualizar el estado del contacto
                                </div>';
                    }
                    ?>
                    <?php
                    if(isset($_GET['success']))
                    {
                        echo '<div class="alert alert-success fade in alert-dismissable" style="margin-top:18px;">
                                        <a href="#" class="close" data-dismiss="alert" aria-label="close" title="close">×</a>
                                        <strong>¡Éxito!</strong> Mensaje respondido correctamente.
                                </div>';
                    }
                    ?>
                    <table class="table table-bordered col-md-12" style="table-layout:fixed;">
                        <thead>
                          <tr>
                            ...
                            <th class="text-center col-md-9">Mensaje</th>
                            <th class="col-md-1"></th>
                          </tr>
                        </thead>
                        <tbody>
                            <?php
                            $adminsMgr = $db->query('SELECT c.timestamp,c.message,c.id FROM contacts c WHERE solved=0');
                            while($row = $adminsMgr->fetch_row())
                            {
                                echo '<tr>
                                        <td class="text-center col-md-2">'.date('d/m/Y H:m:i',$row[0]).'</td>
                                        <td class="text-center col-md-9" style="white-space: nowrap;overflow: hidden;text-overflow: ellipsis">'.$row[1].'</td>
                                        <td class="text-center col-md-1"><a href="'.URL.'/contactresponse?id='.$core->crypt($row[2]).'" class="btn btn-danger">Contestar</a></td>
                                      </tr>';
                            }
                            ?>
                        </tbody>

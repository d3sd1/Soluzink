...
{
    $sessionCode = $core->decrypt($_GET['key']);
    if($sessionCode != '' && $sessionCode != null)
    {
        $sessionInfo = $db->query('SELECT u.name,u.surnames,uu.name,uu.surnames,cm.timestamp,cm.content FROM chat_messages cm JOIN users u ON u.id=cm.sender JOIN users uu ON uu.id=cm.receptor WHERE sessionId="'.$sessionCode.'" ORDER BY timestamp DESC');
        if($sessionInfo->num_rows == 0)
        {
            header('Location: '.URL.'/sessionsdone?error=chatnotfound');
            die();
        }
    }
    else 
        ...
        <div class="container-fluid">
            <div class="page-content page-content-popup">
                ...
                <div class="page-fixed-main-content">
                    <h1>Información del chat</h1>
                    <h2><a href="<?php echo URL ?>/sessionsdone" class="btn btn-primary">Ver sesiones</a></h2>
                    <table class="table table-bordered">
                        ...
                        <tbody>
                            <?php
                            while($row = $sessionInfo->fetch_row())
                            {
                                echo '<tr>

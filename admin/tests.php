        ...
        <div class="container-fluid">
            <div class="page-content page-content-popup">
                ...
                <div class="page-fixed-main-content">
                    ...
                    <h2><a href="<?php echo URL ?>/addtest" class="btn btn-primary">Agregar test</a></h2>
                    <?php
                    if(isset($_GET['del']))
                    {
                        $testId = $core->decrypt($_GET['del']);
                        if($testId != null && $testId != '')
                        {
                            if($db->query('SELECT id FROM tests WHERE id='.$testId)->num_rows > 0)
                            {
                                $db->query('DELETE FROM tests WHERE id='.$testId);
                                $db->query('DELETE FROM tests_questions tq INNER JOIN usersmessages tests_responses tr ON tr.questid=tq.questid WHERE tq.testid='.$testId);
                                echo '<div class="alert alert-success fade in alert-dismissable" style="margin-top:18px;">
                                        <a href="#" class="close" data-dismiss="alert" aria-label="close" title="close">×</a>
                                        <strong>¡Éxito!</strong> Has eliminado el test correctamente.
                                </div>';
                            }
                            else
                            {
                                echo '<div class="alert alert-danger fade in alert-dismissable" style="margin-top:18px;">
                                        <a href="#" class="close" data-dismiss="alert" aria-label="close" title="close">×</a>
                                        <strong>¡Error!</strong> El test seleccionado no es válido.
                                </div>';
                            }
                        }
                        else {
                                echo '<div class="alert alert-danger fade in alert-dismissable" style="margin-top:18px;">
                                        <a href="#" class="close" data-dismiss="alert" aria-label="close" title="close">×</a>
                                        <strong>¡Error!</strong> El test seleccionado no se pudo eliminar.
                                </div>';
                        }
                    }
                    
                    if(isset($_GET['error']) && $_GET['error'] == 'testnotfound')
                    {
                        echo '<div class="alert alert-danger fade in alert-dismissable" style="margin-top:18px;">
                                        <a href="#" class="close" data-dismiss="alert" aria-label="close" title="close">×</a>
                                        <strong>¡Error!</strong> No se encontraron chats para la sesión indicada
                                </div>';
                    }
                    if(isset($_GET['msg']) && $_GET['msg'] == 'testadded')
                    {
                        echo '<div class="alert alert-info fade in alert-dismissable" style="margin-top:18px;">
                                        <a href="#" class="close" data-dismiss="alert" aria-label="close" title="close">×</a>
                                        <strong>¡Éxito!</strong> Se ha agregado el test correctamente.
                                </div>';
                    }
                    ?>
                    <table class="table table-bordered">
                        <thead>
                          <tr>
                            <th class="text-center">Nombre del test</th>
                        ...
                        <tbody>
                            <?php
                            $adminsMgr = $db->query('SELECT langkey,id FROM tests');
                            while($row = $adminsMgr->fetch_row())
                            {
                                echo '<tr>
                                        <td class="text-center">'.$lang['test'][$row[0]].'</td>
                                        <td class="text-center">'.$row[0].'</td>
                                        <td class="text-center"><a href="'.URL.'/modifytest?id='.$core->crypt($row[1]).'" class="btn btn-primary">Modificar test</a></td>
                                        <td class="text-center"><a href="'.URL.'/tests?del='.$core->crypt($row[1]).'" class="btn btn-danger">Eliminar test</a></td>
                                      </tr>';
                            }
                            ?>
                        </tbody>

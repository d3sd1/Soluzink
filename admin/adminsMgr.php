        ...
        <div class="container-fluid">
            <div class="page-content page-content-popup">
                ...
                <div class="page-fixed-main-content">
                    ...
                    <h2><a href="<?php echo URL ?>/addAdminsMgr" class="btn btn-primary">Agregar administrador</a></h2>
                    <?php
                    if(isset($_GET['del']))
                    {
                        $admToDel = $db->query('SELECT usrRange FROM admin_users WHERE id='.$_GET['del']);
                        if($admToDel->num_rows > 0 && $_GET['del'] != ADMIN_ID && $admToDel->fetch_row()[0] < ADMIN_RANGE)
                        {
                            $db->query('INSERT INTO admin_logs (type,ip,userId,timestamp) VALUES ("DELETED_ADMIN",INET_ATON(\''.USER_IP.'\'),(SELECT id FROM admin_users WHERE email="'.ADMIN_EMAIL.'"),"'.time().'")');   
                            $db->query('DELETE FROM admin_users WHERE id='.$_GET['del']);
                            echo '<div class="alert alert-success fade in alert-dismissable" style="margin-top:18px;">
                                    <a href="#" class="close" data-dismiss="alert" aria-label="close" title="close">×</a>
                                    <strong>¡Éxito!</strong> Has eliminado al administrador correctamente.
                            </div>';
                        }
                        else {
                            $db->query('INSERT INTO admin_logs (type,ip,userId,timestamp) VALUES ("DELETED_ADMIN_ERROR",INET_ATON(\''.USER_IP.'\'),(SELECT id FROM admin_users WHERE email="'.ADMIN_EMAIL.'"),"'.time().'")');   
                            echo '<div class="alert alert-danger fade in alert-dismissable" style="margin-top:18px;">
                                    <a href="#" class="close" data-dismiss="alert" aria-label="close" title="close">×</a>
                                    <strong>¡Error!</strong> El administrador introducido no se pudo eliminar.
                            </div>';
                        }
                    }
                    ?>
                    <table class="table table-bordered">
                        ...
                        <tbody>
                            <?php
                            $adminsMgr = $db->query('SELECT user,email,lastIp,geoCity,geoTimeZone,id FROM admin_users');
                            while($row = $adminsMgr->fetch_row())
                            {
                                echo '<tr>
                                        ...
                                        <td class="text-center"><a href="'.URL.'/adminsMgr?del='.$row[5].'" class="btn btn-danger">Eliminar admin</a></td>
                                      </tr>';
                            }
                            ?>
                        </tbody>
